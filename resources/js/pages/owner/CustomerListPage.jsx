...
import { Users, Plus, Search, Pencil, Trash2, Phone, Mail, Hash, DollarSign, Filter, RotateCcw } from 'lucide-react';

const emptyForm = { name: '', phone: '', email: '', address: '' };

export default function CustomerListPage() {
    document.title = 'Customers - M-TAI';
    ...
    const [stats, setStats] = useState({ total: 0, new_this_month: 0, repeat: 0 });
    ...
    const fetchCustomers = useCallback(async () => {
        if (!selectedBusiness) {
            setCustomers([]);
            setTotalCustomers(0);
            setStats({ total: 0, new_this_month: 0, repeat: 0 });
            return;
        }
        setLoading(true);
        try {
            ...
            const res = await api.get(`/owner/businesses/${selectedBusiness}/customers`, { params });
            const data = res.data?.data || [];
            setCustomers(data);
            setCurrentPage(res.data?.current_page || 1);
            setLastPage(res.data?.last_page || 1);
            setTotalCustomers(res.data?.total || data.length);
            if (res.data?.stats) {
    ...
    }, [selectedBusiness, currentPage, search]);
        ...
        <div className="space-y-0">
            {selectedBusiness && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-6">
                    ...
                    <div className="stat-card flex items-center justify-between p-5 bg-white rounded-xl border border-gray-200 shadow-sm">
                        <div>
                            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Repeat Customers</p>
                            <p className="text-2xl font-bold text-gray-900">{stats.repeat}</p>
                        </div>
                        <div className="h-12 w-12 bg-yellow-50 rounded-xl flex items-center justify-center shrink-0">
                            <Hash size={22} className="text-yellow-500" />
                        </div>
                    </div>
                </div>
            )}
            ...
            {!selectedBusiness ? (
                <div className="card overflow-hidden bg-white rounded-2xl border border-gray-200 shadow-sm">
                    <div className="px-6 py-12 text-center">
                        ...
                        <p className="text-sm text-gray-500">Select a business to view customers</p>
                    </div>
                </div>
            ) : loading ? (
                ...
                <div className="card overflow-hidden bg-white rounded-2xl border border-gray-200 shadow-sm mb-6">
                    <div className="px-6 py-4 border-b border-gray-100">
                        <h3 className="text-sm font-semibold text-gray-900">Customers ({customers.length})</h3>
                    </div>
                        ...
                        <table className="w-full">
                            <thead>
                                <tr>
                                    ...
                                    <th className="text-left px-6 py-3 font-medium text-gray-500 text-xs uppercase tracking-wider">Total Spending</th>
                                    ...
                                    <tr key={row.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-3">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <button
                                                    onClick={() => confirmDeleteCustomer(row)}
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            onChange={handleChange}
                            rows={3}
                            className={`${inputClasses} resize-none ${errors.address ? 'border-red-500' : ''}`}
                            placeholder="Enter customer address (optional)"
                        />
                        {errors.address && <p className="mt-1.5 text-sm text-red-600">{errors.address[0]}</p>}
                    </div>
                    ...
                    <div className="flex justify-end space-x-3 pt-2">
                        ...
                        >
                            {submitting ? 'Processing...' : editing ? 'Save Changes' : 'Add Customer'}
                        </button>
                    </div>
                </form>
            </Modal>
            ...
            <ConfirmDialog
                isOpen={confirmOpen}
                onClose={() => { setConfirmOpen(false); setDeleteId(null); setDeleteName(''); }}
                onConfirm={handleDelete}
                title="Delete Customer"
                message={`Are you sure you want to delete customer "${deleteName}"? This action cannot be undone.`}
                confirmText="Delete"
                cancelText="Cancel"
                variant="danger"
            />
