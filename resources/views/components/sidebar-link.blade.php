...
@php
    $activeClass = $active ? 'bg-primary/10 text-primary font-medium' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-800';
@endphp

<a href="{{ $href }}" class="flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm transition {{ $activeClass }}">
    @switch($icon)
        @case('home')
        ...
        @case('user')
            ...
            @break
        @default
            <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 9l3 3m0 0l-3 3m3-3H8m13 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
    @endswitch
    <span>{{ $label }}</span>
</a>
